...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///ar_electronics.db'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['CART_FILE'] = os.path.join(app.config['DATA_DIR'], "cart.json")
db = SQLAlchemy(app)

# Create data directory if it doesn't exist
os.makedirs(app.config['DATA_DIR'], exist_ok=True)
os.makedirs(app.config['STATIC_IMG'], exist_ok=True)
...
def load_cart():
    try:
        with open(app.config['CART_FILE'], 'r') as file:
            return json.load(file)
    except FileNotFoundError:
        return {}

def save_cart(cart):
    save_data(app.config['CART_FILE'], cart)
...
if not os.path.exists(app.config['CART_FILE']):
    save_data(app.config['CART_FILE'], {})
...
def signup():
    if request.method == 'POST':
        ...
        if User.query.filter_by(username=username).first():
            ...
            new_user = User(username=username, email=email, password=hashed_password, name=name, address=address)
...
@login_required
def view_cart():
    cart = load_cart().get(session['username'], {})
    cart_items = []
    total = 0
...
def add_to_cart():
    try:
        ...
        cart = load_cart()
        user_cart = cart.get(session['username'], {})
        key = f"{category}|{brand}|{model}"
        if key in user_cart:
            user_cart[key]['quantity'] += quantity
        else:
            user_cart[key] = {
                'category': category,
                'brand': brand,
        ...
        cart[session['username']] = user_cart
        save_cart(cart)
...
def update_cart():
    ...
    if key in user_cart:
        ...
        user_cart[key]['quantity'] = new_quantity
        cart[session['username']] = user_cart
        save_cart(cart)
...
def checkout():
    cart = load_cart().get(session['username'], {})
    app.logger.debug(f"Cart data for user {session['username']}: {cart}")
    if not cart:
        flash('Your cart is empty. Please add items to your cart before proceeding to checkout.', 'warning')
        return redirect(url_for('view_cart'))
...
def order_history():
    ...
    app.logger.debug(f"User orders: {user_orders}")
...
def profile():
    ...
    order_count = len(orders.get(user.username, []))
    wishlist_count = len(wishlist.get(user.username, []))
...
def upload_profile_photo():
    ...
    if file:
        # Save the uploaded file to the static/img directory
        filename = f"{session['username']}_profile.jpg"
        file_path = os.path.join(app.config['STATIC_IMG'], filename)
        file.save(file_path)
...
def buy():
    cart = load_cart().get(session['username'], {})
    if not cart:
        flash('Your cart is empty. Please add items to your cart before proceeding to buy.', 'warning')
        return redirect(url_for('view_cart'))

    if request.method == 'POST':
        # Get form data
        payment_method = request.form.get('payment_method', 'Cash on Delivery')
        shipping_address = request.form.get('shipping_address', users[session['username']]['address'])
        ...
        # Clear cart
        cart.pop(session['username'], None)
        save_cart(cart)

        # Redirect to order confirmation page
        return redirect(url_for('order_confirmation'))

    return render_template('buy.html', cart=cart, user=users[session['username']])
